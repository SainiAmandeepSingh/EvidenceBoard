...
function clearPanel(){currentRel=null;_backFn=null;document.getElementById('pb').innerHTML='<p class="pp">Click any string to see evidence chain and temporal burst chart.<br>Click any entity card to see connections, activity sparkline, and comm stats.<br>Click a relationship type pill for a focused partner view.<br>Click background to reset focus.</p>';var qs=document.getElementById('qsec');if(qs)renderQueue();}
...
function renderSpoke(d){
  ...
  rels.slice(0,n).forEach(function(rel,i){
    var a=(2*Math.PI*i/n)-Math.PI/2,px=cx+r*Math.cos(a),py=cy+r*Math.sin(a);
    svg.append('line').attr('x1',cx).attr('y1',cy).attr('x2',px).attr('y2',py).attr('stroke','#E24B4A').attr('stroke-width',Math.max(1.5,Math.min(7,rel.ev*0.5+1))).attr('stroke-opacity',0.55);
  });
  rels.slice(0,n).forEach(function(rel,i){
    var a=(2*Math.PI*i/n)-Math.PI/2,px=cx+r*Math.cos(a),py=cy+r*Math.sin(a);
    var g=svg.append('g').style('cursor','pointer');
    ...
    var lR=r+32,lx=cx+lR*Math.cos(a),ly=cy+lR*Math.sin(a);
    var pl=rel.partner.length>14?rel.partner.slice(0,13)+'\u2026':rel.partner;
    var tw=pl.length*5.5+8;
    ...
    g.on('mouseenter',function(ev){d3.select(this).select('circle').attr('fill','#FCEBEB');d3.select(this).select('rect').attr('stroke-opacity',0.9);var content='<strong>'+rel.partner+'</strong><br>Evidence: '+rel.ev;if(rel.comms&&rel.comms.length){var cm=rel.comms[0];content+='<br><span style="color:#aaa;font-size:10px;">'+cm.ts+'  '+cm.from+' to '+cm.to+'</span><br>'+(cm.text||'').slice(0,220)+((cm.text&&cm.text.length>220)?'\u2026':'');}showTip3(content,ev.clientX,ev.clientY);}).on('mouseleave',function(){d3.select(this).select('circle').attr('fill','#fff8f8');d3.select(this).select('rect').attr('stroke-opacity',0.4);hideTip3();});
  });
...
var selectedEntityId=null,barSel=null,scatterSel=null,scatterLblSel=null;
function updateSelection(id){
  ...
  if(scatterSel)scatterSel.attr('r',function(d){return d.id===id?14:9;}).attr('stroke',function(d){return d.id===id?'#1a1a18':'white';}).attr('stroke-width',function(d){return d.id===id?2.5:1.5;});
}
...
function showSuspDetail(d){
  ...
  // Show ALL comms for each relationship (full text 300 chars)
  d.rels.forEach(function(r){
    ...
    (r.comms||[]).forEach(function(c){
      h+='<div style="border-left:2px solid #E24B4A22;padding-left:7px;margin-bottom:5px;">'
        +'<div style="font-size:9px;font-weight:600;color:#534AB7;margin-bottom:2px;">'+c.ts+'  '+c.from+' to '+c.to+'</div>'
        +'<div style="font-size:9.5px;color:#3d3d3a;line-height:1.6;">'+(c.text||'').slice(0,300)+((c.text&&c.text.length>300)?'\u2026':'')+'</div>'
        +'</div>';
    });
    h+='</div>';
  });
...
setTimeout(function(){
  ...
  // Measure exact available chart width — eliminates horizontal scrollbar
  var chartArea=document.getElementById('chart-area');
  var cW=chartArea?chartArea.clientWidth-2:700;
  var ML=188,MR=20,MT=8,BAR_H=30,GAP=7;
  var BAR_W=Math.max(260,cW-ML-MR);
  ...
  // ── Bar chart ───────────────────────────────────────────────────────────────
  var barSvgH=MT+top.length*(BAR_H+GAP)+50;
  ...
  barSel=barG.selectAll('.bg').data(top).join('g').attr('class','bg')
  ...
  barSel.append('text').attr('x',function(d){return ML+scX(d.ev)+6;}).attr('y',BAR_H/2).attr('dominant-baseline','central').attr('font-size','9').attr('fill','#73726c').attr('font-family','system-ui,sans-serif').text(function(d){return d.ev+'ev, '+d.count+'x';});
  ...
  // ── Risk matrix scatter — proper D3 zoom with rescaling axes ───────────────
  var SM={l:60,r:30,t:30,b:50};
  var SW=svgW-SM.l-SM.r;var SH=Math.min(320,window.innerHeight*0.45);
  var scatterSvgH=SM.t+SH+SM.b+60;
  ...
  // Beeswarm x-jitter within each count group
  var countGroups={};
  top.forEach(function(d){if(!countGroups[d.count])countGroups[d.count]=[];countGroups[d.count].push(d);});
  var maxGN=Math.max.apply(null,Object.keys(countGroups).map(function(k){return countGroups[k].length;}));
  var JX=Math.min(15,Math.abs(scX2(1)-scX2(0))*0.38/Math.max(1,(maxGN-1)/2));
  var cvGroups={};
  top.forEach(function(d){var k=d.count+'|'+d.ev;if(!cvGroups[k])cvGroups[k]=[];cvGroups[k].push(d);});
  top.forEach(function(d){
    var cg=countGroups[d.count],ci=cg.indexOf(d),cn=cg.length;d._jx=(ci-(cn-1)/2)*JX;
    var cvg=cvGroups[d.count+'|'+d.ev],cvi=cvg.indexOf(d),cvn=cvg.length;d._jy=(cvi-(cvn-1)/2)*7;
  });
  ...
  for(var li=1;li<lpos.length;li++)for(var lj=0;lj<li;lj++)if(Math.abs(lpos[li].px-lpos[lj].px)<85&&Math.abs(lpos[li].ly-lpos[lj].ly)<LH)lpos[li].ly=lpos[lj].ly+LH+1;
  ...
  // Axis groups (inside scatterG — they scroll with data on zoom)
  ...
  // Style axes
  function styleAxis(g){g.selectAll('.tick text').attr('font-size','8').attr('fill','#888780').attr('font-family','system-ui');g.selectAll('.tick line').attr('stroke','#e8e6e0');g.select('.domain').attr('stroke','#d3d1c7');}
  styleAxis(xAxisG);styleAxis(yAxisG);

  // Axis labels (fixed in SVG, not in axis group)
  scatterG.append('text').attr('x',SM.l+SW/2).attr('y',SM.t+SH+42).attr('text-anchor','middle').attr('font-size','10').attr('fill','#73726c').attr('font-family','system-ui').text('Number of suspicious connections');
  scatterG.append('text').attr('transform','rotate(-90)').attr('x',-(SM.t+SH/2)).attr('y',SM.l-44).attr('text-anchor','middle').attr('font-size','10').attr('fill','#73726c').attr('font-family','system-ui').text('Total evidence');

  // Quadrant annotation (top-right corner, fixed)
  scatterG.append('text').attr('x',SM.l+SW-4).attr('y',SM.t+12).attr('text-anchor','end').attr('font-size','8.5').attr('fill','#E24B4A').attr('opacity',0.55).attr('font-family','system-ui').text('HIGHEST PRIORITY');
  // Quadrant background
  var qX=scX2((maxCnt+0.8)/2),qY=scY2((maxEv+0.8)/2);
  dataG.append('rect').attr('x',qX).attr('y',SM.t).attr('width',SM.l+SW-qX).attr('height',qY-SM.t).attr('fill','#E24B4A').attr('opacity',0.04).attr('pointer-events','none');

  // Entity type colour legend
  var EC={Person:'#1D9E75',Organization:'#534AB7',Vessel:'#185FA5',Group:'#BA7517',Location:'#888780'};
  var legTypes=Object.keys(EC);
  var legX=SM.l, legY=SM.t+SH+54;
  legTypes.forEach(function(t,i){
    scatterG.append('circle').attr('cx',legX+i*90).attr('cy',legY).attr('r',5).attr('fill',EC[t]);
    scatterG.append('text').attr('x',legX+i*90+9).attr('y',legY).attr('dominant-baseline','central').attr('font-size','8.5').attr('fill','#73726c').attr('font-family','system-ui').text(t);
  });
  // Color scatter points by entity type (override conflict coloring)
  // Use conflict overlay instead
  scatterG.append('text').attr('x',SM.l+SW).attr('y',legY).attr('text-anchor','end').attr('font-size','8').attr('fill','#888780').attr('font-family','system-ui').text('Dark ring = conflict entity  \u00B7  Scroll to zoom  \u00B7  Drag to pan');

  // Labels
  scatterLblSel=dataG.selectAll('.ptl').data(top).join('text').attr('class','ptl')
    .attr('x',function(d){return (lById[d.id]?lById[d.id].px:scX2(d.count)+d._jx)+14;})
    .attr('y',function(d){return lById[d.id]?lById[d.id].ly:scY2(d.ev)+d._jy;})
    .attr('font-size','8.5').attr('fill','#3d3d3a').attr('font-family','system-ui').attr('pointer-events','none')
    .text(function(d){return lById[d.id]?lById[d.id].txt:(d.label.length>15?d.label.slice(0,14)+'\u2026':d.label);});

  // Circles — coloured by entity subtype, conflict = dark ring
  scatterSel=dataG.selectAll('.pt').data(top).join('circle').attr('class','pt')
    .attr('cx',function(d){return scX2(d.count)+d._jx;})
    .attr('cy',function(d){return scY2(d.ev)+d._jy;})
    .attr('r',9)
    .attr('fill',function(d){return EC[d.sub]||'#888780';})
    .attr('stroke',function(d){return d.conflict?'#E24B4A':'white';})
    .attr('stroke-width',function(d){return d.conflict?2.5:1.5;})
    .attr('opacity',0.88).style('cursor','pointer')
    .on('mouseenter',function(ev,d){d3.select(this).attr('r',13).attr('opacity',1);showTip3('<strong>'+d.label+'</strong><br>'+d.sub+'<br>Evidence: '+d.ev+'<br>Connections: '+d.count+(d.conflict?' \u26A0 conflict':''),ev.clientX,ev.clientY);})
    .on('mousemove',function(ev){showTip3(tip3.innerHTML,ev.clientX,ev.clientY);})
    .on('mouseleave',function(ev,d){d3.select(this).attr('r',selectedEntityId===d.id?14:9).attr('opacity',0.88);hideTip3();})
    .on('click',function(ev,d){showSuspDetail(d);});

  // ── Proper D3 zoom — axes rescale WITH data on pan/zoom ─────────────────────
  var currentView='bar';
  var scatterZoom=d3.zoom().scaleExtent([0.4,8])
    .filter(function(ev){return currentView==='scatter';})
    .on('zoom',function(ev){
      var t=ev.transform;
      var newX=t.rescaleX(scX2), newY=t.rescaleY(scY2);
      // Rescale axes (they update their ticks to reflect new domain)
      xAxisG.call(xAxisFn.scale(newX));
      yAxisG.call(yAxisFn.scale(newY));
      styleAxis(xAxisG);styleAxis(yAxisG);
      // Update circle and label positions
      scatterSel
        .attr('cx',function(d){return newX(d.count)+d._jx*t.k;})
        .attr('cy',function(d){return newY(d.ev)+d._jy*t.k;});
      scatterLblSel
        .attr('x',function(d){return newX(d.count)+d._jx*t.k+14;})
        .attr('y',function(d){
          var base=newY(d.ev)+d._jy*t.k;
          if(lById[d.id]){var orig=lById[d.id].py;var shift=base-(scX2(top[0]?top[0].count:0));return base+(lById[d.id].ly-orig);}
          return base;
        });
      // Update quadrant
      dataG.select('rect').attr('x',newX((maxCnt+0.8)/2)).attr('y',newY((maxEv+0.8)/2)).attr('width',SM.l+SW-newX((maxCnt+0.8)/2)).attr('height',newY((maxEv+0.8)/2)-SM.t<0?0:newY((maxEv+0.8)/2)-SM.t);
    });
  svg3.call(scatterZoom);

  // Reset zoom button
  var resetBtn=document.getElementById('btn-reset-zoom');
  if(resetBtn)resetBtn.addEventListener('click',function(){svg3.transition().duration(400).call(scatterZoom.transform,d3.zoomIdentity);});
  ...
  function setView(v){
    ...
    if(resetBtn)resetBtn.style.display=isBar?'none':'inline-block';
  }
  var bl=document.getElementById('btn-list'),br=document.getElementById('btn-risk');
  if(bl)bl.addEventListener('click',function(){setView('bar');});
  if(br)br.addEventListener('click',function(){setView('scatter');});

  // ── Sort toggle — Brehmer & Munzner "arrange" interaction ──────────────────
  var sortMode='ev'; // 'ev' or 'conn'
  function applySort(){
    var sorted=top.slice().sort(function(a,b){return sortMode==='ev'?-(a.ev+a.count*2-(b.ev+b.count*2)):-(a.count-b.count||a.ev-b.ev);});
    barSel.data(sorted,function(d){return d.id;}).transition().duration(350)
      .attr('transform',function(d,i){return 'translate(0,'+(MT+i*(BAR_H+GAP))+')';});
  }
  var bsev=document.getElementById('btn-sort-ev'),bsco=document.getElementById('btn-sort-conn');
  if(bsev)bsev.addEventListener('click',function(){sortMode='ev';applySort();bsev.style.background='#BA7517';bsev.style.color='white';bsev.style.border='1px solid #BA7517';if(bsco){bsco.style.background='#fff';bsco.style.color='#3d3d3a';bsco.style.border='1px solid #d3d1c7';}});
  if(bsco)bsco.addEventListener('click',function(){sortMode='conn';applySort();bsco.style.background='#BA7517';bsco.style.color='white';bsco.style.border='1px solid #BA7517';if(bsev){bsev.style.background='#fff';bsev.style.color='#3d3d3a';bsev.style.border='1px solid #d3d1c7';}});

  // ── Evidence filter + entity search ─────────────────────────────────────────
  var evSlider=document.getElementById('ev-filter');
  if(evSlider){evSlider.max=maxEv;evSlider.addEventListener('input',function(){var minEv=parseInt(this.value,10);document.getElementById('ev-val').textContent=minEv;barSel.each(function(d){var v=d.ev>=minEv;d3.select(this).style('opacity',v?1:0.1).style('pointer-events',v?'all':'none');});if(scatterSel)scatterSel.style('opacity',function(d){return d.ev>=minEv?0.88:0.08;});if(scatterLblSel)scatterLblSel.style('opacity',function(d){return d.ev>=minEv?1:0.08;});});}
  var searchEl=document.getElementById('entity-search');
  if(searchEl){searchEl.addEventListener('input',function(){var q=this.value.toLowerCase().trim();barSel.each(function(d){var v=!q||d.label.toLowerCase().indexOf(q)!==-1;d3.select(this).style('opacity',v?1:0.1).style('pointer-events',v?'all':'none');});if(scatterSel)scatterSel.style('opacity',function(d){return(!q||d.label.toLowerCase().indexOf(q)!==-1)?0.88:0.08;});if(scatterLblSel)scatterLblSel.style('opacity',function(d){return(!q||d.label.toLowerCase().indexOf(q)!==-1)?1:0.08;});});}

  setView('bar'); // initial state
},60);
...
<div id="wrap3">
  ...
  <div id="body3">
    ...
    <div id="ctrl-bar" style="padding:7px 14px;background:#f8f7f4;border-bottom:1px solid #eee;display:flex;align-items:center;gap:14px;flex-wrap:wrap;flex-shrink:0;">
      ...
      <div style="display:flex;gap:4px;">
        <button id="btn-sort-ev" style="padding:3px 9px;border-radius:4px;border:1px solid #BA7517;background:#BA7517;color:white;font-size:9.5px;cursor:pointer;font-weight:600;" title="Sort by total evidence">By evidence</button>
        <button id="btn-sort-conn" style="padding:3px 9px;border-radius:4px;border:1px solid #d3d1c7;background:#fff;color:#3d3d3a;font-size:9.5px;cursor:pointer;" title="Sort by connection count">By connections</button>
      </div>
