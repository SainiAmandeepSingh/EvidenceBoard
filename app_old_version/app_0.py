...
# ── Cell 2: load data and compute everything ───────────────────────────────────
# Rule: every name returned here must NOT be imported/defined in any other cell.
# Private names (prefixed _) stay local to this cell only.
@app.cell
def _():
    ...
    _pos = _nx.spring_layout(_G, k=2.6, iterations=150, seed=42, weight="weight")

    _EC = {
        "Person": "#1D9E75", "Organization": "#534AB7",
        "Vessel": "#185FA5", "Group": "#BA7517", "Location": "#888780",
    }
    all_ents = []
    for _en in _ent_nodes:
        _eid = _en["id"]
...
# ── Cell 3: create controls (no .value access here) ───────────────────────────
@app.cell
...
# ── Cell 4: read .value, filter, render ───────────────────────────────────────
# All names here that are not returned are prefixed _ to stay private.
@app.cell
def _(mo, all_ents, all_rels, all_ghosts, MAX_EV,
    ...
    _board = mo.iframe(f"""<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8">
...
<div id="wrap">
  <div id="hdr">
    ...
    <div id="hdr-s">
      <span>{len(_ve)} entities</span>
      <span>{len(_vr)} relationships</span>
      <span>{_nc} conflict{'s' if _nc != 1 else ''}</span>
      <span>{len(_vg)} predicted missing</span>
    </div>
  </div>
  <div id="main">
...
function showGH(g){{
  document.getElementById("pb").innerHTML=
    ...
    "These entities exchanged <strong>"+g.n+" communications</strong> but no relationship node exists in the graph \u2014 a predicted data gap.</div>"+
...
(function draw(){{
  var W=Math.max(window.innerWidth-260, 300);
  var H=Math.max(window.innerHeight-80, 400);
  document.getElementById("pb").innerHTML=
    "<p style="font-size:11px;color:#534AB7;padding:8px;background:#EEEDFE;border-radius:6px;">"
    +"Debug: W="+W+" H="+H+"<br>Entities: "+E.length+"<br>Rels: "+R.length+"</p>";
  try {{
  var svg=d3.select("#svg").attr("width",W).attr("height",H).attr("viewBox","0 0 "+W+" "+H);
  var scX=d3.scaleLinear().domain([-1.15,1.15]).range([22+CW/2,W-22-CW/2]);
  var scY=d3.scaleLinear().domain([-1.15,1.15]).range([36+CH/2,H-14-CH/2]);
  var ep={{}};
  ...
  sl.selectAll(".gs").data(GH).join("path").attr("class","gs").attr("s","0")
    ...
    .on("mouseenter",function(){{
      if(d3.select(this).attr("s")!=="1") d3.select(this).attr("stroke-opacity",0.8).attr("stroke-width",3.5);
    }})
    .on("mouseleave",function(){{
      if(d3.select(this).attr("s")!=="1") d3.select(this).attr("stroke-opacity",0.45).attr("stroke-width",2);
    }})
  ...
  sl.selectAll(".rs").data(R).join("path").attr("class","rs").attr("s","0")
    ...
    .on("mouseenter",function(ev,d){{
      if(d3.select(this).attr("s")!=="1") d3.select(this).attr("stroke-width",sw(d.ev)+2.5).attr("stroke-opacity",1);
    }})
    .on("mouseleave",function(ev,d){{
      if(d3.select(this).attr("s")!=="1") d3.select(this).attr("stroke-width",sw(d.ev)).attr("stroke-opacity",so(d.ev));
    }})
  ...
  var cards=cl.selectAll(".ec").data(E).join("g").attr("class","ec")
    .attr("transform",function(d){{
      var p=ep[d.id]; return p?"translate("+p.lx+","+p.ly+")":"translate(0,0)";
    }});
  ...
  svg.append("text").attr("x",12).attr("y",22)
    ...
    document.getElementById("pb").innerHTML+=
      "<p style="color:#E24B4A;font-size:11px;padding:8px;">JS Error: "+err.message+"</p>";
  }}
}})();
</script>
</body>
