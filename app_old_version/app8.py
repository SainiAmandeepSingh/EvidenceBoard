...
var lastClickedEnt = null;  // stores entity for back-navigation from layer 2
  ...
  ALL_DATES.forEach(function(d,i){
    ...
    if(v>0){
      bar.on('mouseenter',function(ev){
        ...
        if(t){t.innerHTML='<strong>'+d.slice(5)+'</strong><br>'+v+' comm'+(v!==1?'s':'');t.style.display='block';t.style.left=Math.min(ev.clientX+14,window.innerWidth-150)+'px';t.style.top=(ev.clientY-10)+'px';}
      }).on('mouseleave',function(){d3.select(this).attr('opacity',op).attr('stroke',null);var t=document.getElementById('tip');if(t)t.style.display='none';});
    }
    if(d===peakD&&peakV>0) svg.append('text').attr('x',i*(bw+1)+bw/2).attr('y',H-bh-4).attr('text-anchor','middle').attr('font-size','7.5').attr('fill',color||'#1D9E75').attr('font-family','system-ui').text(d.slice(5));
  });
...
function renderBurst(r) {
  ...
  if(!hasData){d3.select(ctr).append('p').style('font-size','10px').style('color','#888780').text('No temporal data.');return;}
  var maxV=1; ALL_DATES.forEach(function(d){if((daily[d]||0)>maxV)maxV=daily[d];});
...
// ── LAYER 2: mini relationship-type network ───────────────────────────────────
// Rendered when analyst clicks a relationship type pill in the entity panel.
// Shows the entity in the centre with all partners of that type as satellite nodes.
// Clicking any partner calls showRel() for that specific relationship.
function renderMiniNetwork(eid, typeRels, col) {
  var ctr=document.getElementById('mini-net-ctr'); if(!ctr) return;
  d3.select(ctr).selectAll('*').remove();
  var n=Math.min(typeRels.length,7);
  if(!n) return;
  ...
  typeRels.slice(0,n).forEach(function(rel,i){
    ...
    var g=svg.append('g').style('cursor','pointer')
      .on('click',function(){showRel(rel);})
      .on('mouseenter',function(ev){
        d3.select(this).select('circle').attr('fill',col+'22');
        var t=document.getElementById('tip');
        if(t){t.innerHTML='<strong>'+pid+'</strong><br>'+rel.ev+' evidence<br><em>Click to see communications</em>';t.style.display='block';t.style.left=Math.min(ev.clientX+14,window.innerWidth-160)+'px';t.style.top=(ev.clientY-10)+'px';}
    ...
    // External label along spoke direction
...
// ── Layer 2 entry: analyst clicks a relationship type pill ───────────────────
function showRelType(eid, typeName) {
  ...
  var partners=typeRels.map(function(r){ return {id:r.ents[0]===eid?r.ents[1]:r.ents[0],ev:r.ev,rel:r}; }).sort(function(a,b){return b.ev-a.ev;});
  ...
  renderMiniNetwork(eid, typeRels, col);
  // Build partner list with D3 for click events
  ...
  if(listCtr){
    var sel=d3.select(listCtr).selectAll('.pl').data(typeRels.slice(0,6)).join('div').attr('class','pl')
      ...
      .style('margin-bottom','6px').style('cursor','pointer').style('transition','background 0.15s')
      .on('click',function(ev,r){showRel(r);})
    ...
    sel.each(function(r){
      var pid=r.ents[0]===eid?r.ents[1]:r.ents[0];
      ...
      if(r.comms&&r.comms.length){
        var c=r.comms[0];
        div.append('div').style('font-size','9px').style('color','#888780').style('margin-top','2px')
          .text(c.ts+'  '+c.from+' to '+c.to);
...
function escalate(){
  ...
  if(!queue.some(function(q){return q.id===currentRel.id;})) queue.push(currentRel);
  renderQueue();
  var b=document.getElementById('qbanner');
  if(b){b.style.display='block';b.textContent='Added to queue ('+queue.length+')';setTimeout(function(){b.style.display='none';},2000);}
}
function removeFromQueue(i){queue.splice(i,1);renderQueue();}
function renderQueue(){
  var el=document.getElementById('qsec');if(!el||!queue.length){if(el)el.innerHTML='';return;}
  var h='<div style="border-top:1px solid #d3d1c7;margin-top:14px;padding-top:10px;"><div style="font-size:10px;font-weight:600;color:#3d3d3a;margin-bottom:6px;">Investigation queue ('+queue.length+')</div>';
  queue.forEach(function(r,i){var col=r.color||'#888780';h+='<div style="background:#f8f7f4;border-radius:5px;padding:6px 9px;margin-bottom:5px;display:flex;justify-content:space-between;align-items:flex-start;"><div><span style="font-size:9.5px;font-weight:600;color:'+col+';">'+r.sub+'</span><div style="font-size:9px;color:#73726c;margin-top:1px;">'+r.ents.join(' and ')+'</div><div style="font-size:8.5px;color:#888780;">Evidence: '+r.ev+'</div></div><button onclick="removeFromQueue('+i+')" style="font-size:13px;color:#888780;background:none;border:none;cursor:pointer;">&times;</button></div>';});
  el.innerHTML=h+'</div>';
}

function clearPanel(){
  currentRel=null;
  document.getElementById('pb').innerHTML='<p class="pp">Click any string to see evidence chain and temporal burst chart.<br>Click any entity card to see connections, activity sparkline, and comm stats.<br>Click a relationship type pill for a focused view of that relationship.<br>Click background to reset focus.</p>';
  var qs=document.getElementById('qsec');if(qs) renderQueue();
}

function showRel(r){
  currentRel=r;var col=r.color||'#888780';
  var pct=Math.round(r.ev/MEV*100);var lbl=pct<30?'Low':pct<65?'Medium':'High';
...
function showEnt(e){
  ...
  var badge=e.conflict?'<div style="margin-top:6px;font-size:9.5px;background:#FCEBEB;border-radius:4px;padding:5px 8px;color:#791F1F;line-height:1.5;">\u26A0 This entity has both a Suspicious and a Colleagues or Friends relationship. This contradiction is a key investigative signal.</div>':'';
  var h='<div style="background:'+col+'18;border:1px solid '+col+'55;border-radius:6px;padding:8px 10px;margin-bottom:10px;">'
  ...
  if(e.types&&e.types.length){
    e.types.forEach(function(t){
      ...
      // Pill is clickable — enters Layer 2
      h+='<span onclick="showRelType(\''+e.id+'\',\''+t+'\')" style="display:inline-block;margin:0 4px 4px 0;padding:3px 8px;border-radius:12px;font-size:9px;font-weight:600;background:'+tc+'20;color:'+tc+';border:1px solid '+tc+'55;cursor:pointer;transition:background 0.15s;" onmouseenter="this.style.background=\''+tc+'40\'" onmouseleave="this.style.background=\''+tc+'20\'">'+t+' \u2192</span>';
    });
  }
  h+='<div id="qsec"></div>';
  document.getElementById('pb').innerHTML=h;
...
// Hour-of-day bar chart: renders into #hour-chart-ctr using D3
function renderHourChart(msgs) {
  ...
  // Axis tick labels: midnight, 06, 12, 18
  [0,6,12,18].forEach(function(h){ svg.append('text').attr('x',h*(bw+1)+bw/2).attr('y',H+13).attr('text-anchor','middle').attr('font-size','8').attr('fill','#888780').attr('font-family','system-ui').text(String(h).padStart(2,'0')+':00'); });
  svg.append('text').attr('x',23*(bw+1)+bw/2).attr('y',H+13).attr('text-anchor','end').attr('font-size','8').attr('fill','#888780').attr('font-family','system-ui').text('23:00');
}
...
setTimeout(function(){
  ...
  // ── Date column headers — clickable for day summary ───────────────────────
  svg.selectAll('.dlbl').data(dates).join('text').attr('class','dlbl')
    .attr('x',function(d,i){return ML+i*cellW+cellW/2;}).attr('y',MT-10)
    .attr('text-anchor','middle').attr('font-size','9').attr('fill','#73726c')
    .attr('font-family','system-ui,sans-serif').style('cursor','pointer')
    ...
    .on('click',function(ev,d){
      // Day summary: rank entities by message count on this date
      var dayItems=entities.map(function(ent){var c=cellMap[ent+'|'+d];return c?{entity:ent,count:c.count,msgs:c.msgs}:null;})
        .filter(function(x){return x&&x.count>0;}).sort(function(a,b){return b.count-a.count;});
      var dtot=DTOTALS[d]||0;
      var h='<div style="font-size:11px;font-weight:600;color:#185FA5;margin-bottom:4px;">'+d+'</div>'
        +'<div style="font-size:10px;color:#888780;margin-bottom:8px;">'+dtot+' total communications across '+dayItems.length+' entities</div>';
      // Mini bar chart of top entities that day
  ...
  svg.selectAll('.cell').data(cd).join('rect').attr('class','cell')
    ...
    .on('mouseenter',function(ev,d){
      ...
      showTip('<strong>'+d.entity+'</strong><br>'+d.date+'<br>'+c.count+' msg'+(c.count!==1?'s':'')+' sent<br><span style="color:#aaa">Day total: '+dtot+'</span>',ev.clientX,ev.clientY);
    })
    ...
    .on('click',function(ev,d){
      var c=cellMap[d.key];if(!c)return;
      // Show ALL messages in a scrollable container + hour-of-day chart
      var h='<div style="font-size:11px;font-weight:600;color:#3d3d3a;margin-bottom:3px;">'+d.entity+' on '+d.date+'</div>'
      ...
      c.msgs.forEach(function(m){
        h+='<div style="background:#f8f7f4;border-radius:5px;padding:6px 8px;margin-bottom:5px;">'
          ...
          +(m.content.length>200?m.content.slice(0,199)+'\u2026':m.content)+'\u201d</div></div>';
      });
      h+='</div>';
      document.getElementById('cpanel').innerHTML=h;
  ...
  // Count labels inside cells with count >= 3
...
function showTip3(html,x,y){tip3.innerHTML=html;tip3.style.display='block';tip3.style.left=Math.min(x+14,window.innerWidth-tip3.offsetWidth-10)+'px';tip3.style.top=(y-10)+'px';}
function hideTip3(){tip3.style.display='none';}

// ── D3 interactive spoke network — fixed external labels ──────────────────────
// Labels rendered OUTSIDE partner circles as white pill backgrounds.
// Partner nodes are hoverable: shows full name, evidence count, and comm excerpt.
function renderSpoke(d) {
  ...
  rels.slice(0,n).forEach(function(rel,i){
    ...
    svg.append('line').attr('x1',cx).attr('y1',cy).attr('x2',px).attr('y2',py).attr('stroke','#E24B4A').attr('stroke-width',sw).attr('stroke-opacity',0.55);
  });
  // Partner nodes with external labels
  rels.slice(0,n).forEach(function(rel,i){
    var a=(2*Math.PI*i/n)-Math.PI/2;
    var px=cx+r*Math.cos(a),py=cy+r*Math.sin(a);
    var g=svg.append('g').style('cursor','pointer');
    // Node circle (small — just indicator dot)
    g.append('circle').attr('cx',px).attr('cy',py).attr('r',10).attr('fill','#fff8f8').attr('stroke','#E24B4A').attr('stroke-width',1.5);
    // Evidence count inside circle
    ...
    // External label: pill outside the circle along spoke direction
    var lR=r+30, lx=cx+lR*Math.cos(a), ly=cy+lR*Math.sin(a);
    var pl=rel.partner.length>13?rel.partner.slice(0,12)+'\u2026':rel.partner;
    var tw=pl.length*5.2+8;
    g.append('rect').attr('x',lx-tw/2).attr('y',ly-7).attr('width',tw).attr('height',14).attr('rx',3).attr('fill','white').attr('stroke','#E24B4A').attr('stroke-opacity',0.35).attr('stroke-width',1);
    g.append('text').attr('x',lx).attr('y',ly).attr('text-anchor','middle').attr('dominant-baseline','central').attr('font-size','8').attr('fill','#3d3d3a').attr('font-family','system-ui').text(pl);
    // Hover: show full name, evidence, full comm excerpt
    g.on('mouseenter',function(ev){
      d3.select(this).select('circle').attr('fill','#FCEBEB');
      d3.select(this).select('rect').attr('stroke-opacity',0.8);
      var content='<strong>'+rel.partner+'</strong><br>Evidence: '+rel.ev;
      if(rel.comms&&rel.comms.length){
        var cm=rel.comms[0];
        content+='<br><span style="color:#aaa;font-size:10px;">'+cm.ts+'  '+cm.from+' to '+cm.to+'</span>'
          +'<br>'+(cm.text||'').slice(0,220)+((cm.text&&cm.text.length>220)?'\u2026':'');
      }
      showTip3(content,ev.clientX,ev.clientY);
    }).on('mouseleave',function(){
      d3.select(this).select('circle').attr('fill','#fff8f8');
      d3.select(this).select('rect').attr('stroke-opacity',0.35);
      hideTip3();
    });
  });
  // Centre node
  svg.append('circle').attr('cx',cx).attr('cy',cy).attr('r',27).attr('fill',d.color).attr('stroke','white').attr('stroke-width',2);
  if(d.conflict) svg.append('circle').attr('cx',cx+21).attr('cy',cy-21).attr('r',6).attr('fill','#E24B4A').attr('stroke','white').attr('stroke-width',1.5);
  var cl=d.label.length>11?d.label.slice(0,10)+'\u2026':d.label;
  svg.append('text').attr('x',cx).attr('y',cy-3).attr('text-anchor','middle').attr('font-size','8.5').attr('font-weight','600').attr('font-family','system-ui').attr('fill','white').text(cl);
  svg.append('text').attr('x',cx).attr('y',cy+8).attr('text-anchor','middle').attr('font-size','7').attr('font-family','system-ui').attr('fill','rgba(255,255,255,0.8)').text(d.sub);
  svg.append('text').attr('x',W/2).attr('y',H-4).attr('text-anchor','middle').attr('font-size','7.5').attr('fill','#888780').attr('font-family','system-ui').text('Spoke width = evidence \u00B7 Hover labels for full details');
}
...
function showSuspDetail(d){
  ...
  d.rels.forEach(function(r){
    h+='<div style="background:#f8f7f4;border-radius:5px;padding:7px 9px;margin-bottom:6px;"><div style="display:flex;justify-content:space-between;margin-bottom:4px;"><span style="font-size:10px;font-weight:600;color:#E24B4A;">with '+r.partner+'</span><span style="font-size:9px;color:#888780;">'+r.ev+' evidence</span></div>';
    if(r.comms&&r.comms.length){var c=r.comms[0];h+='<div style="font-size:9px;font-weight:600;color:#534AB7;margin-bottom:2px;">'+c.ts+'  '+c.from+' to '+c.to+'</div><div style="font-size:9.5px;color:#3d3d3a;line-height:1.5;">\u201c'+(c.text||'').slice(0,150)+((c.text&&c.text.length>150)?'\u2026':'')+'\u201d</div>';}
    h+='</div>';
  });
...
setTimeout(function(){
  ...
  if(!top.length){document.getElementById('chart-area').querySelector('div').innerHTML+='<p style="padding:20px;color:#888780;">No suspicious data found.</p>';return;}
  ...
  var ML=188,MR=50,MT=8,BAR_H=30,GAP=7;
  var aEl=document.getElementById('ssvg').parentNode;
  var cW=aEl?aEl.getBoundingClientRect().width:600;
  var BAR_W=Math.max(280,cW-ML-MR-10);
  var svgW=ML+BAR_W+MR;

  // Update filter slider max
  var sl=document.getElementById('ev-filter');if(sl)sl.max=maxEv;

  // ── Bar chart ─────────────────────────────────────────────────────────────
  var svgH=MT+top.length*(BAR_H+GAP)+50;
  var svg3=d3.select('#ssvg').attr('width',svgW).attr('height',svgH);
  var scX=d3.scaleLinear().domain([0,maxEv]).range([0,BAR_W]);

  var barG=svg3.append('g').attr('id','bar-g');
  var bg=barG.selectAll('.bg').data(top).join('g').attr('class','bg')
  ...
  bg.append('text').attr('x',ML-8).attr('y',BAR_H/2).attr('text-anchor','end').attr('dominant-baseline','central').attr('font-size','10.5').attr('font-family','system-ui,sans-serif').attr('font-weight',function(d){return d.conflict?'600':'400';}).attr('fill',function(d){return d.color||'#3d3d3a';}).text(function(d){return d.label.length>23?d.label.slice(0,22)+'\u2026':d.label;});
  bg.append('text').attr('x',ML-8).attr('y',BAR_H/2+12).attr('text-anchor','end').attr('dominant-baseline','central').attr('font-size','8.5').attr('fill','#888780').attr('font-family','system-ui,sans-serif').text(function(d){return d.sub+(d.conflict?' \u26A0':'');});
  bg.append('rect').attr('class','bar').attr('x',ML).attr('y',4).attr('height',BAR_H-8).attr('rx',4).attr('width',function(d){return Math.max(4,scX(d.ev));}).attr('fill',function(d){return d.conflict?'#E24B4A':'#F5A0A0';});
  bg.append('text').attr('x',function(d){return ML+scX(d.ev)+6;}).attr('y',BAR_H/2).attr('dominant-baseline','central').attr('font-size','9').attr('fill','#73726c').attr('font-family','system-ui,sans-serif').text(function(d){return d.ev+'ev, '+d.count+'x';});
  var axY=MT+top.length*(BAR_H+GAP)+8;
  barG.append('line').attr('x1',ML).attr('x2',ML+BAR_W).attr('y1',axY).attr('y2',axY).attr('stroke','#d3d1c7').attr('stroke-width',.5);
  barG.append('text').attr('x',ML).attr('y',axY+12).attr('font-size','9').attr('fill','#888780').attr('font-family','system-ui,sans-serif').text('0');
  barG.append('text').attr('x',ML+BAR_W).attr('y',axY+12).attr('text-anchor','end').attr('font-size','9').attr('fill','#888780').attr('font-family','system-ui,sans-serif').text('Total suspicious evidence');
  var lY=axY+26;
  barG.append('rect').attr('x',ML).attr('y',lY).attr('width',14).attr('height',10).attr('rx',2).attr('fill','#E24B4A');
  barG.append('text').attr('x',ML+18).attr('y',lY+5).attr('dominant-baseline','central').attr('font-size','9').attr('fill','#73726c').attr('font-family','system-ui,sans-serif').text('Conflict entity (Suspicious AND Colleagues)');
  barG.append('rect').attr('x',ML+240).attr('y',lY).attr('width',14).attr('height',10).attr('rx',2).attr('fill','#F5A0A0');
  barG.append('text').attr('x',ML+258).attr('y',lY+5).attr('dominant-baseline','central').attr('font-size','9').attr('fill','#73726c').attr('font-family','system-ui,sans-serif').text('Suspicious only');

  // ── Risk matrix scatter plot ───────────────────────────────────────────────
  // X = suspicious connections count, Y = total evidence
  // Entities top-right (many connections AND high evidence) are highest priority
  var SM={l:ML,r:60,t:30,b:50};
  var SW=cW-SM.l-SM.r-10;
  var SH=260;
  var scatterH=SM.t+SH+SM.b+30;

  var scatterG=svg3.append('g').attr('id','scatter-g').style('display','none');
  svg3.attr('height',Math.max(svgH,scatterH)); // SVG tall enough for both

  var scX2=d3.scaleLinear().domain([0,maxCount+0.5]).range([SM.l,SM.l+SW]);
  var scY2=d3.scaleLinear().domain([0,maxEv+0.5]).range([SM.t+SH,SM.t]);

  // Quadrant background guides
  var midX2=scX2((maxCount+1)/2), midY2=scY2((maxEv+1)/2);
  scatterG.append('rect').attr('x',midX2).attr('y',SM.t).attr('width',SM.l+SW-midX2).attr('height',SH/2).attr('fill','#E24B4A').attr('opacity',0.04);
  scatterG.append('text').attr('x',SM.l+SW-4).attr('y',SM.t+12).attr('text-anchor','end').attr('font-size','8.5').attr('fill','#E24B4A').attr('opacity',0.5).attr('font-family','system-ui').text('HIGHEST PRIORITY');

  // Axes
  scatterG.append('line').attr('x1',SM.l).attr('x2',SM.l+SW).attr('y1',SM.t+SH).attr('y2',SM.t+SH).attr('stroke','#d3d1c7').attr('stroke-width',.5);
  scatterG.append('line').attr('x1',SM.l).attr('x2',SM.l).attr('y1',SM.t).attr('y2',SM.t+SH).attr('stroke','#d3d1c7').attr('stroke-width',.5);
  // Axis labels
  scatterG.append('text').attr('x',SM.l+SW/2).attr('y',SM.t+SH+36).attr('text-anchor','middle').attr('font-size','10').attr('fill','#73726c').attr('font-family','system-ui').text('Number of suspicious connections');
  scatterG.append('text').attr('x',SM.l-30).attr('y',SM.t+SH/2).attr('text-anchor','middle').attr('transform','rotate(-90,'+(SM.l-30)+','+(SM.t+SH/2)+')').attr('font-size','10').attr('fill','#73726c').attr('font-family','system-ui').text('Total evidence');
  // Tick marks
  for(var xi=0;xi<=maxCount;xi++){scatterG.append('text').attr('x',scX2(xi)).attr('y',SM.t+SH+14).attr('text-anchor','middle').attr('font-size','8').attr('fill','#888780').attr('font-family','system-ui').text(xi);}
  for(var yi=0;yi<=maxEv;yi+=2){scatterG.append('text').attr('x',SM.l-6).attr('y',scY2(yi)).attr('text-anchor','end').attr('dominant-baseline','central').attr('font-size','8').attr('fill','#888780').attr('font-family','system-ui').text(yi);}

  // Scatter points
  scatterG.selectAll('.pt').data(top).join('circle').attr('class','pt')
    .attr('cx',function(d){return scX2(d.count);})
    .attr('cy',function(d){return scY2(d.ev);})
    .attr('r',9).attr('fill',function(d){return d.conflict?'#E24B4A':'#F5A0A0';})
    .attr('stroke','white').attr('stroke-width',1.5).attr('opacity',0.85)
    .style('cursor','pointer')
    .on('mouseenter',function(ev,d){d3.select(this).attr('r',12).attr('opacity',1);showTip3('<strong>'+d.label+'</strong><br>Evidence: '+d.ev+'<br>Connections: '+d.count+(d.conflict?' \u26A0 conflict':''),ev.clientX,ev.clientY);})
    .on('mousemove',function(ev){showTip3(tip3.innerHTML,ev.clientX,ev.clientY);})
    .on('mouseleave',function(){d3.select(this).attr('r',9).attr('opacity',0.85);hideTip3();})
    .on('click',function(ev,d){showSuspDetail(d);});

  // Point labels (positioned right of circle)
  scatterG.selectAll('.ptl').data(top).join('text').attr('class','ptl')
    .attr('x',function(d){return scX2(d.count)+12;})
    .attr('y',function(d){return scY2(d.ev)+3;})
    .attr('font-size','8.5').attr('fill','#3d3d3a').attr('font-family','system-ui')
    .attr('pointer-events','none')
    .text(function(d){return d.label.length>14?d.label.slice(0,13)+'\u2026':d.label;});

  // ── Evidence filter slider ─────────────────────────────────────────────────
  var evFilter=document.getElementById('ev-filter');
  if(evFilter){
    evFilter.max=maxEv;
    evFilter.addEventListener('input',function(){
      var minEv=parseInt(this.value,10);
      document.getElementById('ev-val').textContent=minEv;
      barG.selectAll('.bg').each(function(d){var vis=d.ev>=minEv;d3.select(this).style('opacity',vis?1:0.12).style('pointer-events',vis?'all':'none');});
      scatterG.selectAll('.pt,.ptl').each(function(d){d3.select(this).style('opacity',d.ev>=minEv?0.85:0.1);});
    });
  }
  ...
  function setView(v){
    currentView=v;
    var isBar=v==='bar';
    barG.style('display',isBar?null:'none');
    scatterG.style('display',isBar?'none':null);
    svg3.attr('height',isBar?svgH:scatterH);
    document.getElementById('btn-list').style.background=isBar?'#534AB7':'#fff';
    document.getElementById('btn-list').style.color=isBar?'white':'#3d3d3a';
    document.getElementById('btn-list').style.border=isBar?'1px solid #534AB7':'1px solid #d3d1c7';
    document.getElementById('btn-risk').style.background=isBar?'#fff':'#534AB7';
    document.getElementById('btn-risk').style.color=isBar?'#3d3d3a':'white';
    document.getElementById('btn-risk').style.border=isBar?'1px solid #d3d1c7':'1px solid #534AB7';
  }
  ...
  <div id="body3">
    ...
    <div id="ctrl-bar" style="padding:7px 14px;background:#f8f7f4;border-bottom:1px solid #eee;display:flex;align-items:center;gap:14px;flex-wrap:wrap;flex-shrink:0;">
      ...
      <div style="display:flex;align-items:center;gap:6px;font-size:11px;color:#73726c;">
        ...
        <input type="range" id="ev-filter" min="1" max="30" value="1" style="width:90px;accent-color:#534AB7;">
      </div>
    </div>
    <div style="flex:1;overflow:auto;"><svg id="ssvg"></svg></div>
  </div>
