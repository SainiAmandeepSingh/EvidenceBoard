...
var selectedEntityId=null,barG=null,barSel=null,scatterSel=null,scatterLblSel=null;
...
function showSuspDetail(d){
  ...
  d.rels.forEach(function(r){
    ...
    (r.comms||[]).forEach(function(c){
      h+='<div style="border-left:2px solid #E24B4A33;padding-left:7px;margin-bottom:5px;">'
        +'<div style="font-size:9px;font-weight:600;color:#534AB7;margin-bottom:2px;">'+c.ts+'  '+c.from+' to '+c.to+'</div>'
        +'<div style="font-size:9.5px;color:#3d3d3a;line-height:1.6;">'+(c.text||'').slice(0,300)+(c.text&&c.text.length>300?'\u2026':'')+'</div></div>';
    });
    h+='</div>';
  });
...
function getSuspDaily(d){
  var daily={};
  ALL_DATES3.forEach(function(dt){daily[dt]=0;});
  d.rels.forEach(function(r){
    (r.comms||[]).forEach(function(c){
      var dt=(c.ts||'').slice(0,10);
      if(daily.hasOwnProperty(dt))daily[dt]++;
    });
  });
  return ALL_DATES3.map(function(dt){return daily[dt];});
}
...
setTimeout(function(){
  ...
  // Exact available width via clientWidth — eliminates horizontal scroll
  var chartArea=document.getElementById('chart-area');
  var cW=(chartArea?chartArea.clientWidth:900)-2;

  // Layout: ML (labels) | bars | gap | ev-label | gap | sparkline | MR
  var ML=188, SP_W=62, MR=16;
  var LABEL_GAP=58;  // space reserved for evidence label after bar
  var SP_GAP=10;     // gap between ev-label and sparkline
  var BAR_W=cW-ML-LABEL_GAP-SP_GAP-SP_W-MR;
  if(BAR_W<180)BAR_W=180;
  var svgW=ML+BAR_W+LABEL_GAP+SP_GAP+SP_W+MR; // exactly cW

  var MT=16,BAR_H=30,GAP=7;
  var maxEv_=maxEv||1;
  var scX=d3.scaleLinear().domain([0,maxEv_]).range([0,BAR_W]);
  ...
  barG=svg3.append('g').attr('id','bar-g');

  // Sparkline column header
  var spX0=ML+BAR_W+LABEL_GAP+SP_GAP;
  barG.append('text').attr('x',spX0+SP_W/2).attr('y',MT-4).attr('text-anchor','middle').attr('font-size','7.5').attr('fill','#888780').attr('font-family','system-ui').text('Susp. by day');
  ...
  // Evidence label (floats after bar end, capped so it doesn't overlap sparkline)
  barSel.append('text')
    .attr('x',function(d){return Math.min(ML+scX(d.ev)+6, ML+BAR_W+LABEL_GAP-4);})
    .attr('y',BAR_H/2).attr('dominant-baseline','central').attr('font-size','9').attr('fill','#73726c').attr('font-family','system-ui,sans-serif')
    .text(function(d){return d.ev+'ev, '+d.count+'x';});

  // Suspicious activity sparkline (14-day mini histogram per entity)
  var spN=ALL_DATES3.length,spBw=Math.max(1,Math.floor((SP_W)/spN)-1);
  var spBarMax=BAR_H-10;
  barSel.each(function(d){
    var counts=getSuspDaily(d);
    var maxC=Math.max(1,Math.max.apply(null,counts));
    var g=d3.select(this);
    counts.forEach(function(v,i){
      var bh=v>0?Math.max(2,Math.round(v/maxC*spBarMax)):1;
      g.append('rect')
        .attr('x',spX0+i*(spBw+1)).attr('y',4+spBarMax-bh)
        .attr('width',spBw).attr('height',bh)
        .attr('fill',v>0?'#E24B4A':'#f1efe8')
        .attr('opacity',v>0?(0.30+v/maxC*0.65):1).attr('rx',0.5);
    });
    // Hover overlay for sparkline peak date
    var peakIdx=counts.indexOf(Math.max.apply(null,counts));
    var peakDate=peakIdx>=0?ALL_DATES3[peakIdx]:'';
    g.append('rect').attr('x',spX0).attr('y',3).attr('width',SP_W).attr('height',spBarMax+2).attr('fill','transparent').style('cursor','pointer')
      .on('mouseenter',function(ev){showTip3('<strong>'+d.label+'</strong><br>Suspicious activity<br>Peak: '+(peakDate?peakDate.slice(8)+'-'+peakDate.slice(5,7):'—'),ev.clientX,ev.clientY);})
      .on('mouseleave',function(){hideTip3();});
  });
  ...
  // Sparkline legend
  barG.append('rect').attr('x',spX0).attr('y',lY).attr('width',10).attr('height',10).attr('rx',1).attr('fill','#E24B4A').attr('opacity',0.6);
  barG.append('text').attr('x',spX0+14).attr('y',lY+5).attr('dominant-baseline','central').attr('font-size','9').attr('fill','#73726c').attr('font-family','system-ui,sans-serif').text('Daily suspicious evidence');

  // ── Risk matrix scatter ─────────────────────────────────────────────────────
  var SM={l:60,r:30,t:30,b:55};
  var SW=svgW-SM.l-SM.r;var SH=Math.min(300,window.innerHeight*0.42);
  ...
  // Beeswarm: x-jitter within count groups, y-jitter for same (count,ev)
  var cGroups={};
  top.forEach(function(d){if(!cGroups[d.count])cGroups[d.count]=[];cGroups[d.count].push(d);});
  var maxGN=Math.max.apply(null,Object.keys(cGroups).map(function(k){return cGroups[k].length;}));
  var countSpacing=Math.abs(scX2(1)-scX2(0));
  var JX=Math.min(14,countSpacing*0.36/Math.max(1,(maxGN-1)/2));
  var cvGrps={};
  top.forEach(function(d){var k=d.count+'|'+d.ev;if(!cvGrps[k])cvGrps[k]=[];cvGrps[k].push(d);});
  top.forEach(function(d){
    var cg=cGroups[d.count],ci=cg.indexOf(d),cn=cg.length;d._jx=(ci-(cn-1)/2)*JX;
    var cvg=cvGrps[d.count+'|'+d.ev],cvi=cvg.indexOf(d),cvn=cvg.length;d._jy=(cvi-(cvn-1)/2)*11;
  });

  // Non-overlapping label positions (top-down sweep)
  var LH=12,LW=90;
  ...
  function styleAxes(){
    [xAxisG,yAxisG].forEach(function(g){
      g.selectAll('.tick text').attr('font-size','8').attr('fill','#888780').attr('font-family','system-ui');
      g.selectAll('.tick line').attr('stroke','#e8e6e0');
      g.select('.domain').attr('stroke','#d3d1c7');
    });
  }
  styleAxes();
  ...
  // Quadrant dashed median lines — update on zoom
  var qMidX0=scX2((maxCnt+0.8)/2),qMidY0=scY2((maxEv+0.8)/2);
  var qLineV=axG.append('line').attr('x1',qMidX0).attr('y1',SM.t).attr('x2',qMidX0).attr('y2',SM.t+SH).attr('stroke','#c8c6be').attr('stroke-width',1).attr('stroke-dasharray','5,4');
  var qLineH=axG.append('line').attr('x1',SM.l).attr('y1',qMidY0).attr('x2',SM.l+SW).attr('y2',qMidY0).attr('stroke','#c8c6be').attr('stroke-width',1).attr('stroke-dasharray','5,4');

  // Quadrant labels (fixed corners, always visible)
  var QL=[
    {x:SM.l+5,       y:SM.t+14,     txt:'High evidence',   a:'start', col:'#888780', w:400},
    {x:SM.l+SW-4,    y:SM.t+14,     txt:'HIGHEST PRIORITY',a:'end',   col:'#E24B4A', w:600},
    {x:SM.l+5,       y:SM.t+SH-6,   txt:'Low risk',        a:'start', col:'#888780', w:400},
    {x:SM.l+SW-4,    y:SM.t+SH-6,   txt:'Many connections',a:'end',   col:'#888780', w:400},
  ];
  QL.forEach(function(q){
    scatterG.append('text').attr('x',q.x).attr('y',q.y).attr('text-anchor',q.a).attr('font-size','8.5').attr('fill',q.col).attr('opacity',q.w===600?0.65:0.45).attr('font-family','system-ui').attr('font-weight',q.w===600?'600':'400').text(q.txt);
  });

  // Quadrant background (top-right tint, in dataG so it zooms with data)
  var qBg=dataG.append('rect').attr('x',qMidX0).attr('y',SM.t).attr('width',SM.l+SW-qMidX0).attr('height',qMidY0-SM.t).attr('fill','#E24B4A').attr('opacity',0.04).attr('pointer-events','none');
  ...
  // Circles — coloured by entity type, conflict = red ring
  ...
  scatterSel=dataG.selectAll('.pt').data(top).join('circle').attr('class','pt')
    ...
    .attr('r',9).attr('fill',function(d){return EC[d.sub]||'#888780';})
    ...
    .on('mouseenter',function(ev,d){d3.select(this).attr('r',13).attr('opacity',1);showTip3('<strong>'+d.label+'</strong><br>'+d.sub+'<br>Ev: '+d.ev+' &nbsp; Conn: '+d.count+(d.conflict?' \u26A0':''),ev.clientX,ev.clientY);})
    .on('mousemove',function(ev){showTip3(tip3.innerHTML,ev.clientX,ev.clientY);})
    .on('mouseleave',function(ev,d){d3.select(this).attr('r',selectedEntityId===d.id?14:9).attr('opacity',0.88);hideTip3();})
    .on('click',function(ev,d){showSuspDetail(d);});

  // Entity type legend + instructions
  var legTypes=Object.keys(EC);
  var legX=SM.l,legY=SM.t+SH+57;
  legTypes.forEach(function(t,i){
    scatterG.append('circle').attr('cx',legX+i*90+5).attr('cy',legY).attr('r',5).attr('fill',EC[t]);
    scatterG.append('text').attr('x',legX+i*90+14).attr('y',legY).attr('dominant-baseline','central').attr('font-size','8.5').attr('fill','#73726c').attr('font-family','system-ui').text(t);
  });
  ...
  // ── Proper D3 zoom: axes AND data update together ────────────────────────────
  var currentView='bar';
  var scatterZoom=d3.zoom().scaleExtent([0.3,10])
    ...
    .on('zoom',function(ev){
      ...
      qLineV.attr('x1',qmx).attr('x2',qmx);
      qLineH.attr('y1',qmy).attr('y2',qmy);
      qBg.attr('x',qmx).attr('y',qmy<SM.t?SM.t:qmy).attr('width',SM.l+SW-qmx<0?0:SM.l+SW-qmx).attr('height',qmy-SM.t<0?0:qmy-SM.t);
      // Points
      scatterSel.attr('cx',function(d){return newX(d.count)+d._jx*t.k;}).attr('cy',function(d){return newY(d.ev)+d._jy*t.k;});
      // Labels
      scatterLblSel.attr('x',function(d){return newX(d.count)+d._jx*t.k+14;}).attr('y',function(d){
        var baseY=newY(d.ev)+d._jy*t.k;
        if(lById[d.id]){var origY=lById[d.id].py,shift=lById[d.id].ly-origY;return baseY+shift;}
        return baseY;
      });
    });
  ...
  function setView(v){
    ...
    if(!isBar)svg3.call(scatterZoom.transform,d3.zoomIdentity); // reset on entry
  ...
  var rz=document.getElementById('btn-reset-zoom');
  if(rz)rz.addEventListener('click',function(){svg3.transition().duration(400).call(scatterZoom.transform,d3.zoomIdentity);});

  // ── Sort toggle: pure ev vs pure connections ───────────────────────────────────
  var sortMode='ev';
  function applySort(){
    var sorted=top.slice().sort(function(a,b){
      return sortMode==='ev'?(b.ev-a.ev||b.count-a.count):(b.count-a.count||b.ev-a.ev);
    });
    // Re-select from DOM to ensure correct data rebinding
    barG.selectAll('.bg').data(sorted,function(d){return d.id;})
      .transition().duration(420)
      .attr('transform',function(d,i){return 'translate(0,'+(MT+i*(BAR_H+GAP))+')';});
  }
  var bse=document.getElementById('btn-sort-ev'),bsc=document.getElementById('btn-sort-conn');
  if(bse)bse.addEventListener('click',function(){
    sortMode='ev';applySort();
    bse.style.background='#BA7517';bse.style.color='white';bse.style.border='1px solid #BA7517';
    if(bsc){bsc.style.background='#fff';bsc.style.color='#3d3d3a';bsc.style.border='1px solid #d3d1c7';}
  });
  if(bsc)bsc.addEventListener('click',function(){
    sortMode='conn';applySort();
    bsc.style.background='#BA7517';bsc.style.color='white';bsc.style.border='1px solid #BA7517';
    if(bse){bse.style.background='#fff';bse.style.color='#3d3d3a';bse.style.border='1px solid #d3d1c7';}
  });

  // ── Evidence filter + entity search ──────────────────────────────────────────
  var evSl=document.getElementById('ev-filter');
  if(evSl){evSl.max=maxEv;evSl.addEventListener('input',function(){var m=parseInt(this.value,10);document.getElementById('ev-val').textContent=m;barG.selectAll('.bg').each(function(d){var v=d.ev>=m;d3.select(this).style('opacity',v?1:0.1).style('pointer-events',v?'all':'none');});if(scatterSel)scatterSel.style('opacity',function(d){return d.ev>=m?0.88:0.08;});if(scatterLblSel)scatterLblSel.style('opacity',function(d){return d.ev>=m?1:0.08;});});}
  var srch=document.getElementById('entity-search');
  if(srch){srch.addEventListener('input',function(){var q=this.value.toLowerCase().trim();barG.selectAll('.bg').each(function(d){var v=!q||d.label.toLowerCase().indexOf(q)!==-1;d3.select(this).style('opacity',v?1:0.1).style('pointer-events',v?'all':'none');});if(scatterSel)scatterSel.style('opacity',function(d){return(!q||d.label.toLowerCase().indexOf(q)!==-1)?0.88:0.08;});if(scatterLblSel)scatterLblSel.style('opacity',function(d){return(!q||d.label.toLowerCase().indexOf(q)!==-1)?1:0.08;});});}
