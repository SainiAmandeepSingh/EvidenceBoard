...
// ── Spoke network — larger centre, external pill labels ───────────────────────
function renderSpoke(d){
  ...
  rels.slice(0,n).forEach(function(rel,i){
    ...
    // External pill label — positioned outside the node circle
    ...
    g.on('mouseenter',function(ev){
      ...
      d3.select(this).select('rect').attr('stroke-opacity',0.9);
      var content='<strong>'+rel.partner+'</strong><br>Evidence: '+rel.ev;
      if(rel.comms&&rel.comms.length){var cm=rel.comms[0];content+='<br><span style="color:#aaa;font-size:10px;">'+cm.ts+'  '+cm.from+' to '+cm.to+'</span><br>'+(cm.text||'').slice(0,220)+((cm.text&&cm.text.length>220)?'\u2026':'');}
      showTip3(content,ev.clientX,ev.clientY);
    }).on('mouseleave',function(){d3.select(this).select('circle').attr('fill','#fff8f8');d3.select(this).select('rect').attr('stroke-opacity',0.4);hideTip3();});
  });
  // Centre node — larger radius with two text lines
...
var barSel=null,scatterSel=null,scatterLblSel=null;
...
setTimeout(function(){
  ...
  // Use window dimensions for reliable width — right panel is 280px
  var cW=window.innerWidth-300;
  var ML=188,MR=30,MT=8,BAR_H=30,GAP=7;
  var BAR_W=Math.max(300,cW-ML-MR);
  var svgW=ML+BAR_W+MR;
  var barSvgH=MT+top.length*(BAR_H+GAP)+50;
  var svg3=d3.select('#ssvg').attr('width',svgW);
  ...
  // ── Risk matrix scatter — beeswarm X + zoom/pan + linked selection ─────────
  var SM={l:ML,r:40,t:30,b:50};
  var SW=BAR_W;var SH=300;
  var scatterSvgH=SM.t+SH+SM.b+20;

  var scatterG=svg3.append('g').attr('id','scatter-g').style('display','none');
  var scatterAxisG=scatterG.append('g');
  var scatterDataG=scatterG.append('g'); // this layer gets zoomed

  // Clip-path prevents data from spilling outside axes
  var clipId='scp'+Date.now();
  var clipDef=svg3.append('defs').append('clipPath').attr('id',clipId);
  clipDef.append('rect').attr('x',SM.l+1).attr('y',SM.t).attr('width',SW-2).attr('height',SH);
  scatterDataG.attr('clip-path','url(#'+clipId+')');

  var scX2=d3.scaleLinear().domain([0,maxCnt+1]).range([SM.l,SM.l+SW]);
  var scY2=d3.scaleLinear().domain([0,maxEv+1]).range([SM.t+SH,SM.t]);

  // Beeswarm: group by count, spread within group along x
  var countGroups={};
  top.forEach(function(d){if(!countGroups[d.count])countGroups[d.count]=[];countGroups[d.count].push(d);});
  var maxGroupN=Math.max.apply(null,Object.keys(countGroups).map(function(k){return countGroups[k].length;}));
  var countSpacing=Math.abs(scX2(1)-scX2(0));
  var JX=Math.min(16,countSpacing*0.38/Math.max(1,(maxGroupN-1)/2));

  // Also handle same (count,ev) collisions with tiny y jitter
  var cvGroups={};
  ...
  var LH=11,LW=88;
  var lpos=top.map(function(d){return{id:d.id,px:scX2(d.count)+d._jx,py:scY2(d.ev)+d._jy,lx:scX2(d.count)+d._jx+15,ly:scY2(d.ev)+d._jy,txt:d.label.length>15?d.label.slice(0,14)+'\u2026':d.label};});
  lpos.sort(function(a,b){return a.py-b.py;});
  for(var li=1;li<lpos.length;li++){for(var lj=0;lj<li;lj++){if(Math.abs(lpos[li].lx-lpos[lj].lx)<LW&&Math.abs(lpos[li].ly-lpos[lj].ly)<LH)lpos[li].ly=lpos[lj].ly+LH+1;}}
  var lposById={};lpos.forEach(function(l){lposById[l.id]=l;});

  // Axes (fixed — not zoomed)
  scatterAxisG.append('line').attr('x1',SM.l).attr('x2',SM.l+SW).attr('y1',SM.t+SH).attr('y2',SM.t+SH).attr('stroke','#d3d1c7').attr('stroke-width',.5);
  scatterAxisG.append('line').attr('x1',SM.l).attr('x2',SM.l).attr('y1',SM.t).attr('y2',SM.t+SH).attr('stroke','#d3d1c7').attr('stroke-width',.5);
  scatterAxisG.append('text').attr('x',SM.l+SW/2).attr('y',SM.t+SH+36).attr('text-anchor','middle').attr('font-size','10').attr('fill','#73726c').attr('font-family','system-ui').text('Number of suspicious connections');
  scatterAxisG.append('text').attr('transform','rotate(-90)').attr('x',-(SM.t+SH/2)).attr('y',SM.l-32).attr('text-anchor','middle').attr('font-size','10').attr('fill','#73726c').attr('font-family','system-ui').text('Total evidence');
  for(var xi=0;xi<=maxCnt;xi++)scatterAxisG.append('text').attr('x',scX2(xi)).attr('y',SM.t+SH+14).attr('text-anchor','middle').attr('font-size','8').attr('fill','#888780').attr('font-family','system-ui').text(xi);
  for(var yi=0;yi<=maxEv;yi+=Math.max(1,Math.floor(maxEv/6)))scatterAxisG.append('text').attr('x',SM.l-6).attr('y',scY2(yi)).attr('text-anchor','end').attr('dominant-baseline','central').attr('font-size','8').attr('fill','#888780').attr('font-family','system-ui').text(yi);

  // Quadrant highlight (top-right = highest priority)
  var qmx=scX2((maxCnt+1)/2),qmy=scY2((maxEv+1)/2);
  scatterDataG.append('rect').attr('x',qmx).attr('y',SM.t).attr('width',SM.l+SW-qmx).attr('height',qmy-SM.t).attr('fill','#E24B4A').attr('opacity',0.04).attr('pointer-events','none');
  scatterAxisG.append('text').attr('x',SM.l+SW-4).attr('y',SM.t+12).attr('text-anchor','end').attr('font-size','8.5').attr('fill','#E24B4A').attr('opacity',0.55).attr('font-family','system-ui').text('HIGHEST PRIORITY');

  // Labels (rendered below circles for z-order)
  scatterLblSel=scatterDataG.selectAll('.ptl').data(top).join('text').attr('class','ptl')
    .attr('x',function(d){return lposById[d.id]?lposById[d.id].lx:scX2(d.count)+15;})
    .attr('y',function(d){return lposById[d.id]?lposById[d.id].ly:scY2(d.ev);})
    .attr('font-size','8.5').attr('fill','#3d3d3a').attr('font-family','system-ui').attr('pointer-events','none')
    .text(function(d){return d.label.length>15?d.label.slice(0,14)+'\u2026':d.label;});

  // Circles
  scatterSel=scatterDataG.selectAll('.pt').data(top).join('circle').attr('class','pt')
    ...
    .style('cursor','pointer')
    .on('mouseenter',function(ev,d){d3.select(this).attr('r',13).attr('opacity',1);showTip3('<strong>'+d.label+'</strong><br>Evidence: '+d.ev+'<br>Connections: '+d.count+(d.conflict?' \u26A0':''),ev.clientX,ev.clientY);})
    .on('mousemove',function(ev){showTip3(tip3.innerHTML,ev.clientX,ev.clientY);})
    .on('mouseleave',function(ev,d){d3.select(this).attr('r',selectedEntityId===d.id?14:9).attr('opacity',0.88);hideTip3();})
    .on('click',function(ev,d){showSuspDetail(d);});

  // Zoom/pan on scatter — only active when scatter is visible
  var currentView='bar';
  var scatterZoom=d3.zoom().scaleExtent([0.5,6])
    .filter(function(ev){return currentView==='scatter';})
    .on('zoom',function(ev){scatterDataG.attr('transform',ev.transform);});
  ...
  // Evidence filter — affects both bar and scatter
  ...
  if(evSlider){
    ...
    evSlider.addEventListener('input',function(){
      ...
      barSel.each(function(d){var vis=d.ev>=minEv;d3.select(this).style('opacity',vis?1:0.1).style('pointer-events',vis?'all':'none');});
      if(scatterSel)scatterSel.style('opacity',function(d){return d.ev>=minEv?0.88:0.08;});
      if(scatterLblSel)scatterLblSel.style('opacity',function(d){return d.ev>=minEv?1:0.08;});
    });
  }

  // Entity search — filter bar and scatter
  var searchEl=document.getElementById('entity-search');
  if(searchEl){
    searchEl.addEventListener('input',function(){
      var q=this.value.toLowerCase().trim();
      barSel.each(function(d){var vis=!q||d.label.toLowerCase().indexOf(q)!==-1;d3.select(this).style('opacity',vis?1:0.1).style('pointer-events',vis?'all':'none');});
      if(scatterSel)scatterSel.style('opacity',function(d){return(!q||d.label.toLowerCase().indexOf(q)!==-1)?0.88:0.08;});
      if(scatterLblSel)scatterLblSel.style('opacity',function(d){return(!q||d.label.toLowerCase().indexOf(q)!==-1)?1:0.08;});
    });
  }
},60);
  ...
  <div id="body3">
    ...
    <div id="ctrl-bar" style="padding:7px 14px;background:#f8f7f4;border-bottom:1px solid #eee;display:flex;align-items:center;gap:14px;flex-wrap:wrap;flex-shrink:0;">
      ...
      <div style="display:flex;align-items:center;gap:6px;font-size:11px;color:#73726c;">
        ...
        <input type="range" id="ev-filter" min="1" max="30" value="1" style="width:90px;accent-color:#534AB7;"><input id="entity-search" type="text" placeholder="Search…" style="margin-left:8px;width:120px;padding:3px 7px;border:1px solid #d3d1c7;border-radius:4px;font-size:10px;outline:none;">
      </div>
    </div>
