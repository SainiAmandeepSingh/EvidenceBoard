...
// Evidence encoding (Ware 2004)
function strokeW(ev){ return Math.max(2,Math.min(10,ev*0.6+1.5)); }
// Confidence encoding (MacEachren 2012)
function strokeO(ev){ return Math.max(0.35,Math.min(0.95,ev/MEV*2+0.35)); }
function trunc(s,n){ return s&&s.length>n?s.slice(0,n-1)+'\u2026':(s||''); }

// ── Activity sparkline: 14-bar mini chart for entity daily comms ──────────────
function makeSparkline(eid, color) {
  var stats  = ESTATS[eid] || {};
  var daily  = stats.daily || {};
  if (!ALL_DATES.length) return '';
  var maxV = 1;
  ALL_DATES.forEach(function(d){ if((daily[d]||0)>maxV) maxV=daily[d]; });
  var w=232, h=40, n=ALL_DATES.length;
  var bw = Math.max(2, Math.floor((w-n)/n));
  var parts = ['<svg width="'+w+'" height="'+h+'" style="display:block;">'];
  ALL_DATES.forEach(function(d,i){
    var v=daily[d]||0;
    var bh=v>0?Math.max(3,Math.round(v/maxV*(h-8))):2;
    var op=v>0?(0.45+v/maxV*0.55):0.12;
    var fill=v>0?(color||'#1D9E75'):'#e0ddd6';
    parts.push('<rect x="'+(i*(bw+1))+'" y="'+(h-bh-4)+'" width="'+bw+'"'
      +' height="'+bh+'" fill="'+fill+'" opacity="'+op.toFixed(2)+'" rx="1"/>');
  });
  // Start and end date labels
  parts.push('<text x="0" y="'+h+'" font-size="7.5" fill="#888780" font-family="system-ui">'+ALL_DATES[0].slice(5)+'</text>');
  parts.push('<text x="'+w+'" y="'+h+'" text-anchor="end" font-size="7.5" fill="#888780" font-family="system-ui">'+ALL_DATES[ALL_DATES.length-1].slice(5)+'</text>');
  parts.push('</svg>');
  return parts.join('');
}

// ── Temporal burst chart: when a relationship's evidence comms happened ─────────
function makeBurst(r) {
  ...
  var maxV=1;
  ALL_DATES.forEach(function(d){ if((daily[d]||0)>maxV) maxV=daily[d]; });
  var w=232, h=38, n=ALL_DATES.length;
  var bw=Math.max(2,Math.floor((w-n)/n));
  var col=r.color||'#888780';
  var parts=['<svg width="'+w+'" height="'+h+'" style="display:block;">'];
  ALL_DATES.forEach(function(d,i){
    var v=daily[d]||0;
    var bh=v>0?Math.max(3,Math.round(v/maxV*(h-6))):2;
    parts.push('<rect x="'+(i*(bw+1))+'" y="'+(h-bh-4)+'" width="'+bw+'"'
      +' height="'+bh+'" fill="'+col+'" opacity="'+(v>0?0.85:0.1)+'" rx="1"/>');
  });
  parts.push('<text x="0" y="'+h+'" font-size="7.5" fill="#888780" font-family="system-ui">'+ALL_DATES[0].slice(5)+'</text>');
  parts.push('<text x="'+w+'" y="'+h+'" text-anchor="end" font-size="7.5" fill="#888780" font-family="system-ui">'+ALL_DATES[ALL_DATES.length-1].slice(5)+'</text>');
  parts.push('</svg>');
...
function clearPanel() {
  ...
  document.getElementById('pb').innerHTML=
    ...
    +'Click any entity card to see connections and activity.<br>'
...
function showEnt(e) {
  ...
  var h='<div style="background:'+col+'18;border:1px solid '+col+'55;border-radius:6px;padding:8px 10px;margin-bottom:10px;">'
    ...
    // Sparkline
    +'<div style="font-size:10px;font-weight:600;color:#3d3d3a;margin-bottom:4px;">Daily communication activity (Oct 1-14)</div>'
    +spark
    +'<div style="margin-top:10px;font-size:10px;font-weight:600;margin-bottom:6px;">Relationship types</div>';
...
"""
    ...
    # ════════════════════════════════════════════════════════════════════════
    # PAGE 2 JS — Timeline heatmap with daily totals bar and row hover
    # ════════════════════════════════════════════════════════════════════════
    _p2_js = r"""
...
// Layout — cap cellW to keep chart within viewport
var ML=175, MT=55, MR=24, TOTALS_H=42, LEG_H=20;
var availW=Math.max(400, window.innerWidth-ML-MR-280);
...
// Colour scale: luminance encodes ordered quantity (Ware 2004)
var colScale=d3.scaleSequential().domain([0,maxCount]).interpolator(d3.interpolate('#e8e6e0','#085041'));

// Max daily total for scaling the totals bar
...
// ── Entity row labels with conflict markers ───────────────────────────────────
var labelGroups=svg.selectAll('.elbl').data(entities).join('g').attr('class','elbl')
  ...
  .on('mouseenter',function(ev,d){
    ...
    svg.append('rect').attr('class','rhl')
      ...
      .attr('fill','#1D9E75').attr('opacity',0.09)
...
// ── Daily totals bar chart (below heatmap) ────────────────────────────────────
// Shows total communication volume per day across all 25 entities
...
dates.forEach(function(d,i){
  ...
  svg.append('rect')
    .attr('x',ML+i*cellW+2).attr('y',dtY+(TOTALS_H-bh-4))
    .attr('width',cellW-4).attr('height',bh).attr('rx',2)
    .attr('fill','#185FA5').attr('opacity',0.6);
  if(v>0)
    svg.append('text')
      .attr('x',ML+i*cellW+cellW/2).attr('y',dtY+TOTALS_H+2)
      .attr('text-anchor','middle')
      .attr('font-size','8').attr('fill','#888780').attr('font-family','system-ui,sans-serif')
      .text(v);
});
...
"""
    ...
    # ════════════════════════════════════════════════════════════════════════
    # PAGE 3 JS — Suspicion analysis with mini spoke network
    # ════════════════════════════════════════════════════════════════════════
    _p3_js = r"""
...
// ── Mini spoke network for selected entity ─────────────────────────────────────
// Shows the entity in the centre with suspicious partners on spokes.
// Spoke width encodes evidence count (Ware 2004 pre-attentive channel).
function makeSpokeGraph(d) {
  var rels=d.rels||[];
  var n=Math.min(rels.length,8);
  if(!n) return '';
  var cx=110, cy=90, r=62;
  var parts=['<svg width="220" height="185" style="display:block;margin:0 auto;">'];
  // Spokes first (drawn behind nodes)
  rels.slice(0,n).forEach(function(rel,i){
    var angle=(2*Math.PI*i/n)-Math.PI/2;
    var px=cx+r*Math.cos(angle), py=cy+r*Math.sin(angle);
    var sw=Math.max(1.5,Math.min(6,rel.ev*0.4+1));
    parts.push('<line x1="'+cx+'" y1="'+cy+'" x2="'+px+'" y2="'+py
      +'" stroke="#E24B4A" stroke-width="'+sw+'" stroke-opacity="0.65"/>');
  });
  // Partner nodes
  rels.slice(0,n).forEach(function(rel,i){
    var angle=(2*Math.PI*i/n)-Math.PI/2;
    var px=cx+r*Math.cos(angle), py=cy+r*Math.sin(angle);
    parts.push('<circle cx="'+px+'" cy="'+py+'" r="13" fill="#f8f7f4" stroke="#d3d1c7" stroke-width="1"/>');
    var pl=rel.partner.length>8?rel.partner.slice(0,7)+'\u2026':rel.partner;
    parts.push('<text x="'+px+'" y="'+(py-2)+'" text-anchor="middle" dominant-baseline="central"'
      +' font-size="6.5" font-family="system-ui" fill="#3d3d3a">'+pl+'</text>');
    parts.push('<text x="'+px+'" y="'+(py+7)+'" text-anchor="middle" dominant-baseline="central"'
      +' font-size="6" font-family="system-ui" fill="#888780">'+rel.ev+'ev</text>');
  });
  // Centre node
  parts.push('<circle cx="'+cx+'" cy="'+cy+'" r="27" fill="'+d.color+'" stroke="white" stroke-width="2"/>');
  if(d.conflict)
    parts.push('<circle cx="'+(cx+21)+'" cy="'+(cy-21)+'" r="6" fill="#E24B4A" stroke="white" stroke-width="1.5"/>');
  var cl=d.label.length>11?d.label.slice(0,10)+'\u2026':d.label;
  parts.push('<text x="'+cx+'" y="'+(cy-4)+'" text-anchor="middle" font-size="8.5"'
    +' font-weight="600" font-family="system-ui" fill="white">'+cl+'</text>');
  parts.push('<text x="'+cx+'" y="'+(cy+8)+'" text-anchor="middle" font-size="7"'
    +' font-family="system-ui" fill="rgba(255,255,255,0.8)">'+d.sub+'</text>');
  parts.push('</svg>');
  return '<div style="margin:10px 0 8px;">'
    ...
    +parts.join('')
    +'<div style="font-size:8.5px;color:#888780;text-align:center;margin-top:2px;">Spoke width encodes evidence count</div>'
    +'</div>';
}

function showSuspDetail(d){
  var col=d.color||'#888780';
  var spoke=makeSpokeGraph(d);
  var h='<div style="background:'+col+'18;border:1px solid '+col+'55;border-radius:6px;padding:8px 10px;margin-bottom:10px;">'
    ...
    +spoke
    +'<div style="font-size:10px;font-weight:600;margin-bottom:6px;">Relationship details</div>';
...
// Use setTimeout to ensure DOM layout is ready before measuring widths
setTimeout(function(){
  ...
  // Subtype and conflict marker below name
  ...
  // Bar — red for conflict entities, lighter for suspicious-only
