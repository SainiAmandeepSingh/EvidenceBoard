...
def _():
    ...
    # Load the MC3 knowledge graph JSON with 1,159 nodes, 3,226 edges
    ...
    for _rn in _rel_nodes:
        ...
        for _cid in _comms[:8]:
            ...
            _cdets.append({
                ...
                "inf":  bool(_cn.get("is_inferred", False)),
            })
...
def _(
    ...
    // ──────────────── Investigation queue ────────────────
    // Queue persists across reactive re-renders via sessionStorage
    var _QK='eb_queue_v1';
    function _saveQueue(){try{sessionStorage.setItem(_QK,JSON.stringify(queue));}catch(e){}}
    function _loadQueue(){try{var s=sessionStorage.getItem(_QK);if(s)queue=JSON.parse(s);}catch(e){}}
    var queue=[],currentRel=null;
    _loadQueue();
    function escalate(){if(!currentRel)return;if(!queue.some(function(q){return q.id===currentRel.id;}))queue.push(currentRel);_saveQueue();renderQueue();var b=document.getElementById('qbanner');if(b){b.style.display='block';b.textContent='Added to queue ('+queue.length+')';setTimeout(function(){b.style.display='none';},2000);}}
    function removeFromQueue(i){queue.splice(i,1);_saveQueue();renderQueue();}
    ...
    <div id="hdr-s">
      <span>__EC__ entities</span><span>__RC__ relationships</span>
      <span>__CC__ conflicts</span><span>__GC__ predicted missing</span>
    </div>
    <div id="hdr-b">
    ...
    _p1_html = (_p1_html
        .replace("__EC__", str(len(_ve)))
        .replace("__RC__", str(len(_vr)))
        .replace("__CC__", str(_nc))
        .replace("__GC__", str(len(_vg)))
